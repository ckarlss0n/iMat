...
public class ProfilePanel extends ScrollPane{
	...
	public void checkTextFields(){
		if(firstNameField.getText().matches("^[A-ZÅÄÖa-zåäö]+([-][A-ZÅÄÖa-zåäö]+)*?(\\s[A-ZÅÄÖa-zåäö]*)*?") 
				&& lastNameField.getText().matches("^[A-ZÅÄÖa-zåäö]+([-][A-ZÅÄÖa-zåäö]+)*?(\\s[A-ZÅÄÖa-zåäö]*)*?") 
				&& emailField.getText().matches("^[a-zA-Z0-9+_.-]+[@][a-zA-Z0-9_-]+\\.([a-zA-Z0-9+_.-]*)?[a-zA-Z0-9+_-]") //kunna ta in "+" framför "@"?
				&& addressField.getText().matches("^[A-ZÅÄÖa-zåäö]+([-][A-ZÅÄÖa-zåäö]+)*?(\\s[A-ZÅÄÖa-zåäö]*)*?(\\s\\d+$)*?") 
				&& postalCodeField.getText().matches("[0-9]{5}")
				&& cityField.getText().matches("^[A-ZÅÄÖa-zåäö]+([-][A-ZÅÄÖa-zåäö]+)*?(\\s[A-ZÅÄÖa-zåäö]*)*?") 
